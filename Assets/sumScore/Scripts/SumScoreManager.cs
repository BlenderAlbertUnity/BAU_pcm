...
public class SumScoreManager : MonoBehaviour {
    ...
    public Text[] fields; // Text field displaying current score
    ...
    void Start() {
        ...
        if (storeHighScore) {
			if (PrefsManager.HasKey("sumHS")) { 
                // Set high score value and tell manager
				SumScore.HighScore = PrefsManager.GetValue<int>("sumHS");
                UpdatedHS();
            }
            else
    ...
    public void Updated () {
		foreach (Text txt in fields) {
			txt.text = SumScore.Score.ToString("0"); // Post new score to text field
		}
    }
